    ...
    public void run() {
        ...
        if (super.isRightMouseState()) {// fire
            if (fireCooldown <= 0) {
                ...
                if (deltaX > 0) {
                    super.getManagerThread().summonFireBall(pX + PLAYER_WIDTH, pY + PLAYER_HEIGHT / 4,new Vector(deltaX / distance * FIREBALL_SPEED ,deltaY / distance * FIREBALL_SPEED));
        ...
        if (super.isLeftMouseState()) {// Melee (reused code from fireball thats why this is bad)
            if (meleeCooldown <= 0) {
                double deltaX = super.getMouseX() - (pX + PLAYER_WIDTH / 2);
                if (deltaX > 0) {
                    super.getManagerThread().summonMelee(pX + PLAYER_WIDTH, pY + PLAYER_HEIGHT / 4,new Vector(1 ,0));//melee facing riht
                } else {// 20 is ball width
        ...
        for (Platform r : ManagerThread.walls) {// collision with walls
            ...
            if (pY + PLAYER_HEIGHT < wY + ManagerThread.MAXVELOCITY && pY + PLAYER_HEIGHT >= wY - .1 && pX + PLAYER_WIDTH > wX && pX < wX + wW) {
            ...
            } else if (pY < wY + ManagerThread.MAXVELOCITY && pY + PLAYER_HEIGHT > wY + 2 && pX < wX && pX + PLAYER_WIDTH > wX) {
                // TODO: touching left edge
                if (v.getXDirection() >= 0) {
                    v.setXDirection(0);
                    super.setXpos(wX - PLAYER_WIDTH);
                }
            } else if (pY < wY + ManagerThread.MAXVELOCITY && pY + PLAYER_HEIGHT > wY + 2 && pX < wX + wW && pX + PLAYER_WIDTH > wX + wW) {
